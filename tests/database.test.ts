/* eslint-disable test/require-to-throw-message */
/* eslint-disable test/no-hooks */
...
describe('database Integration Tests', () => {
  ...
  describe('indexes', () => {
    it('creates unique index', async () => {
      const { indexes, table } = createTable({
        columns: [
          { name: 'id', schema: z.int() },
          { name: 'email', schema: z.email() },
          { name: 'username', schema: z.string() },
        ],
        indexes: [{ columns: ['email'], name: 'idx_accounts_email', unique: true }],
        name: 'accounts',
        primaryKeys: ['id'],
      })
      ...
      const { table } = createTable({
        columns: [
          ...
          { name: 'name', schema: z.string().min(3)
            .max(50) },
          { name: 'age', schema: z.int().min(18)
            .max(65) },
          { name: 'department', schema: z.enum([
            'HR',
            'IT',
            'Sales',
          ]) },
